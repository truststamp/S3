...
function _formatXML(quietSetting, errorResults, deleted) {
    ...
    deleted.forEach(version => {
        ...
        const deletingDeleteMarker = version.deletingDeleteMarker;
...
export function getObjMetadataAndDelete(authInfo, canonicalID, request,
    ...
    return async.forEachLimit(inPlay, 5, (entry, moveOn) => {
        async.waterfall([
            ...
            let isDeleteMarker;
            let deleteMarker;
            // If trying to delete an object that does not exist (empty objMD)
            // return DeleteMarkerVersionId equals the versionID generated and
            // DeleteMarker equals true
            if (!objMD) {
                isDeleteMarker = true;
                deleteMarker = versionIdUtils.encode(versionId);
            // If trying to delete a delete marker, DeleteMarkerVersionId equals
            // deleteMarker's versionID and DeleteMarker equals true
            } else if (objMD.isDeleteMarker) {
                isDeleteMarker = true;
                deleteMarker = entry.versionId;
            } else {
            ...
            successfullyDeleted.push({ entry, newDeleteMarker,
                isDeletingDeleteMarker: isDeleteMarker,
                deletingDeleteMarker: deleteMarker });
            return moveOn();
        });
    },
    // end of forEach func
