...
export function getObjMetadataAndDelete(authInfo, canonicalID, request,
    ...
    return async.forEachLimit(inPlay, 5, (entry, moveOn) => {
        const opts = {
            versionId: (entry.versionId === 'null' ? '' : entry.versionId),
        };
        let objMD = undefined;
        metadata.getObjectMD(bucketName, entry.key, opts, log, (err, obj) => {
            ...
            return async.waterfall([
                callback => {
                    // get the latest version or a version other than 'null'
                    if (entry.versionId !== 'null') {
                        return callback();
                    }
                    // the latest version is 'null'
                    if (objMD.isNull || objMD.versionId === undefined) {
                        return callback();
                    }
                    // the version 'null' does not exist
                    if (objMD.nullVersionId === undefined) {
                        successfullyDeleted.push({ entry });
                        return moveOn();
                    }
                    // the version 'null' exists, go and get it
                    opts.versionId = objMD.nullVersionId;
                    return metadata.getObjectMD(bucketName, entry.key, opts,
                        log, (err, obj) => {
                            if (err && !err.NoSuchKey) {
                                errorResults.push({ entry, error: err });
                                return moveOn();
