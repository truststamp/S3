...
class BucketFileInterface {
    ...
    internalListObject(bucketName, params, log, cb) {
        ...
        const delimiterOnly = listingType === 'Delimiter';
        const extension = new arsenal.algorithms.list[listingType](params, log);
        const listingParams = extension.genMDParams();
        if (delimiterOnly && listingParams.gt) {
            listingParams.gt += String.fromCharCode(1);
        }
        this.loadDBIfExists(bucketName, log, (err, db) => {
            if (err) {
                return cb(err);
            ...
            let cbDone = false;
            let lastKey = delimiterOnly ? params.marker : undefined;
            let isNull = false;
            const stream = db.createReadStream(listingParams);
            stream
                .on('data', e => {
                    ...
                    const vid = this.gc[gcName];
                    if (vid) {
                        if (vid < versionId && versionId !== VID_INF) {
                            return null;
                        }
                    }
                    if (key !== lastKey) {
                        lastKey = key;
                        isNull = JSON.parse(e.value).isNull ? true : false;
                        if (extension.filter({ key, value: e.value }) < 0) {
                            stream.emit('end');
                            return stream.destroy();
                        }
                        if (delimiterOnly || listingType === 'MPU') {
                            return null;
                        }
                    }
                    if (JSON.parse(e.value).isNull && isNull) {
                        return null;
                    }
                    isNull = true;
