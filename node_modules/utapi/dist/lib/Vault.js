...
var _vaultclient = require('vaultclient');

var _vaultclient2 = _interopRequireDefault(_vaultclient);
...
class Vault {
    ...
    constructor(config) {
        ...
        if (config.https) {
            ...
            this._client = new _vaultclient2.default.Client(host, port, true, key, cert, ca);
        } else {
            this._client = new _vaultclient2.default.Client(host, port);
        }
    }
    ...
    */
    authenticateV4Request(params, requestContexts, callback) {
        ...
        this._client.verifySignatureV4(stringToSign, signatureFromRequest, accessKey, region, scopeDate, { reqUid: log.getSerializedUids(),
            requestContext: serializedRCs }, (err, authInfo) => {
            if (err) {
                log.trace('error from vault', { error: err });
                return callback(err);
            }
            return callback(null, authInfo.message.body.authorizationResults);
        });
    }
    ...
    */
    getCanonicalIds(accountIds, log, callback) {
        ...
        return this._client.getCanonicalIdsByAccountIds(accountIds, { reqUid: log.getSerializedUids(), logger: log }, callback);
    }
