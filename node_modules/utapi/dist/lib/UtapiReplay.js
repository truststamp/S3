...
var _UtapiClient = require('./UtapiClient');

var _UtapiClient2 = _interopRequireDefault(_UtapiClient);
...
var _safeJsonParse = require('../utils/safeJsonParse');
...
class UtapiReplay {
     ...
     */
    constructor(config) {
        this.log = new _werelogs.Logger('UtapiReplay', {
            level: 'info',
            dump: 'error'
        });
        ...
        if (config) {
            ...
            const message = 'missing required property in UtapiReplay ' + 'configuration';
            (0, _assert2.default)(config.redis, `${message}: redis`);
            (0, _assert2.default)(config.localCache, `${message}: localCache`);
            this.utapiClient = new _UtapiClient2.default(config);
    ...
    _validateElement(data) {
        ...
        if (!action || !reqUid || !params || !timestamp) {
            this.log.fatal('missing required parameter in element', { method: 'UtapiReplay._validateElement' });
            return false;
        }
        return true;
    }
    ...
    _pushCachedMetrics(metricsArr, cb) {
        return _async2.default.each(metricsArr, (arr, next) => {
            ...
            if (!actionErr && element) {
                const { error, result } = (0, _safeJsonParse2.default)(element);
                if (error) {
                    this.log.error('cannot parse element into JSON', { method: 'UtapiReplay._pushCachedMetrics' });
                    return next(error);
                }
                if (!this._validateElement(result)) {
                    return next();
                }
                this.log.trace('pushing metric with utapiClient::pushMetric', { method: 'UtapiReplay._pushCachedMetrics' });
    ...
    _getCachedMetrics(listLen) {
        ...
        return _async2.default.timesSeries(count, (n, next) => {
    ...
    start() {
        ...
        const replay = (0, _nodeSchedule.scheduleJob)(this.replaySchedule, () => {
