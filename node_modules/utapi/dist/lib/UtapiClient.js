...
class UtapiClient {
     ...
     */
    constructor(config) {
        this.log = new _werelogs.Logger('UtapiClient', {
            level: 'info',
            dump: 'error'
        });
        ...
        if (config) {
            ...
            if (config.metrics) {
                ...
                (0, _assert2.default)(Array.isArray(config.metrics), `${message}: metrics ` + 'must be an array');
                (0, _assert2.default)(config.metrics.length !== 0, `${message}: metrics ` + 'array cannot be empty');
                this.metrics = config.metrics;
            }
            if (config.redis) {
                this.ds = new _Datastore2.default().setClient((0, _redisClient2.default)(config.redis, this.log));
            }
            if (config.localCache) {
                this.localCache = new _Datastore2.default().setClient((0, _redisClient2.default)(config.localCache, this.log));
            }
            if (config.component) {
    ...
    */
    _pushLocalCache(params, operation, timestamp, log, cb) {
        ...
        const action = operation === 'listBucketMultipartUploads' ? 'listMultipartUploads' : operation;
        const logObject = { method: 'UtapiClient._pushLocalCache', action,
            params };
        if (!this.localCache) {
            log.fatal('failed to push metrics', logObject);
            return cb(_arsenal.errors.InternalError);
        }
    ...
    _checkProperties(params, properties = []) {
        properties.forEach(prop => {
            (0, _assert2.default)(params[prop] !== undefined, 'Metric object must include ' + `${prop} property`);
            if (prop === 'oldByteLength') {
                (0, _assert2.default)(typeof params[prop] === 'number' || params[prop] === null, 'oldByteLength  property must be ' + 'an integer or `null`');
            } else {
                (0, _assert2.default)(typeof params[prop] === 'number', `${prop} property ` + 'must be an integer');
            }
        });
    }
    ...
    _checkMetricTypes(params) {
        ...
        this.metrics.forEach(level => {
            ...
            (0, _assert2.default)(typeof params[propName] === 'string' || params[propName] === undefined, `${propName} must be a string`);
        });
    }
    ...
    _getParamsArr(params) {
        ...
        const props = [];
        const { byteLength, newByteLength, oldByteLength, numberOfObjects } = params;
    ...
    */
    _genericPushMetric(params, timestamp, action, log, callback) {
        ...
        const cmds = this._getParamsArr(params).map(p => ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
        return this.ds.batch(cmds, err => {
            if (err) {
                ...
                return this._pushLocalCache(params, action, timestamp, log, callback);
            }
            return callback();
        });
    }
    ...
    */
    _pushMetricUploadPart(params, timestamp, action, log, callback) {
        ...
        paramsArr.forEach(p => {
            cmds.push(['incrby', (0, _schema.generateCounter)(p, 'storageUtilizedCounter'), storageUtilizedDelta], ['incrby', (0, _schema.generateKey)(p, 'incomingBytes', timestamp), newByteLength], ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
        });
        // update counters
        return this.ds.batch(cmds, (err, results) => {
            ...
            const noErr = paramsArr.every((p, i) => {
                ...
                actionCounter = actionCounter < 0 ? storageUtilizedDelta : actionCounter;
                if (actionErr) {
                    ...
                    this._pushLocalCache(params, action, timestamp, log, callback);
                    return false;
                }
                cmds2.push(['zremrangebyscore', (0, _schema.generateStateKey)(p, 'storageUtilized'), timestamp, timestamp], ['zadd', (0, _schema.generateStateKey)(p, 'storageUtilized'), timestamp, actionCounter]);
                return true;
            });
            if (noErr) {
    ...
    * @return {undefined}
    */
    _pushMetricCompleteMultipartUpload(params, timestamp, action, log, callback) {
        this._checkProperties(params);
        this._logMetric(params, '_pushMetricCompleteMultipartUpload', timestamp, log);
        const paramsArr = this._getParamsArr(params);
        const cmds = [];
        paramsArr.forEach(p => {
            cmds.push(['incr', (0, _schema.generateCounter)(p, 'numberOfObjectsCounter')], ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
        });
        ...
        return this.ds.batch(cmds, (err, results) => {
            ...
            const noErr = paramsArr.every((p, i) => {
                ...
                if (actionErr) {
                    log.error('error incrementing counter for push metric', {
                        method: 'UtapiClient._pushMetricCompleteMultipart' + 'Upload',
                        metric: 'number of objects',
                        error: actionErr
                    });
                    this._pushLocalCache(params, action, timestamp, log, callback);
                ...
                key = (0, _schema.generateStateKey)(p, 'numberOfObjects');
                cmds2.push(['zremrangebyscore', key, timestamp, timestamp], ['zadd', key, timestamp, actionCounter]);
                return true;
            });
            if (noErr) {
                return this.ds.batch(cmds2, callback);
    ...
    * @return {undefined}
    */
    _pushMetricListBucketMultipartUploads(params, timestamp, action, log, callback) {
        return this._genericPushMetric(params, timestamp, 'listBucketMultipartUploads', log, callback);
    }
    ...
    */
    _genericPushMetricDeleteObject(params, timestamp, action, log, callback) {
        const expectedProps = action === 'abortMultipartUpload' ? ['byteLength'] : ['byteLength', 'numberOfObjects'];
        this._checkProperties(params, expectedProps);
        const { byteLength, numberOfObjects } = params;
        this._logMetric(params, '_genericPushMetricDeleteObject', timestamp, log);
        const paramsArr = this._getParamsArr(params);
        const cmds = [];
        // We push Redis commands to be executed in batch. The type of commands
        // to push depends on the action.
        paramsArr.forEach(p => {
            cmds.push(['decrby', (0, _schema.generateCounter)(p, 'storageUtilizedCounter'), byteLength], ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
            ...
            if (action !== 'abortMultipartUpload') {
                cmds.push(['decrby', (0, _schema.generateCounter)(p, 'numberOfObjectsCounter'), numberOfObjects]);
            }
        });
        // We track the number of commands needed for each `paramsArr` object to
        // eventually locate each group in the results from Redis.
        ...
        return this.ds.batch(cmds, (err, results) => {
            ...
            const noErr = paramsArr.every((p, i) => {
                ...
                // Sorted Set keys are updated with the value stored at their
                // respective counter key (i.e., the value of `actionCounter`).
                cmds2.push(['zremrangebyscore', (0, _schema.generateStateKey)(p, 'numberOfObjects'), timestamp, timestamp], ['zadd', (0, _schema.generateStateKey)(p, 'numberOfObjects'), timestamp, actionCounter]);
                return true;
            });
            if (noErr) {
                return this.ds.batch(cmds2, callback);
    ...
    */
    _pushMetricGetObject(params, timestamp, action, log, callback) {
        ...
        const cmds = [];
        paramsArr.forEach(p => {
            cmds.push(['incrby', (0, _schema.generateKey)(p, 'outgoingBytes', timestamp), newByteLength], ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
        });
        // update counters
        return this.ds.batch(cmds, err => {
            if (err) {
    ...
    */
    _genericPushMetricPutObject(params, timestamp, action, log, callback) {
        ...
        const paramsArr = this._getParamsArr(params);
        paramsArr.forEach(p => {
            cmds.push(['incrby', (0, _schema.generateCounter)(p, 'storageUtilizedCounter'), storageUtilizedDelta], [redisCmd, (0, _schema.generateCounter)(p, 'numberOfObjectsCounter')], ['incr', (0, _schema.generateKey)(p, action, timestamp)]);
            if (action === 'putObject') {
                cmds.push(['incrby', (0, _schema.generateKey)(p, 'incomingBytes', timestamp), newByteLength]);
            }
        });
        ...
        return this.ds.batch(cmds, (err, results) => {
            ...
            const noErr = paramsArr.every((p, i) => {
                ...
                actionCounter = Number.isNaN(actionCounter) || actionCounter < 0 ? 1 : actionCounter;
                if (actionErr) {
                    log.error('error incrementing counter for push metric', {
                        method: 'UtapiClient._genericPushMetricPutObject',
                        metric: 'number of objects',
