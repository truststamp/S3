...
var _s3metricResponse = require('../../models/s3metricResponse');

var _s3metricResponse2 = _interopRequireDefault(_s3metricResponse);
...
class ListMetrics {
    ...
    constructor(metric, component) {
        ...
        this.vault = new _Vault2.default(_Config2.default);
    }
    ...
    _getMetricResponse(resource, start, end) {
        ...
        const metricResponse = JSON.parse(JSON.stringify(_s3metricResponse2.default));
    ...
    getTypesMetrics(utapiRequest, cb) {
        ...
        if (this.metric === 'accounts') {
            return this.vault.getCanonicalIds(resources, log, (err, list) => {
                ...
                return _async2.default.mapLimit(list.message.body, 5, (item, next) => this.getMetrics(item.canonicalId, timeRange, datastore, log, (err, res) => {
                    if (err) {
                        return next(err);
                    }
                    return next(null, Object.assign({}, res, { accountId: item.accountId }));
                }), cb);
            });
        }
        return _async2.default.mapLimit(resources, 5, (resource, next) => this.getMetrics(resource, timeRange, datastore, log, next), cb);
    }
    ...
    */
    getRecentTypesMetrics(utapiRequest, cb) {
        ...
        const datastore = utapiRequest.getDatastore();
        _async2.default.mapLimit(resources, 5, (resource, next) => this.getMetrics(resource, timeRange, datastore, log, next), cb);
    }
    ...
    */
    getMetrics(resource, range, datastore, log, cb) {
        ...
        const storageUtilizedKey = (0, _schema.generateStateKey)(obj, 'storageUtilized');
        const numberOfObjectsKey = (0, _schema.generateStateKey)(obj, 'numberOfObjects');
        const storageUtilizedStart = ['zrevrangebyscore', storageUtilizedKey, start, '-inf', 'LIMIT', '0', '1'];
        const storageUtilizedEnd = ['zrevrangebyscore', storageUtilizedKey, end, '-inf', 'LIMIT', '0', '1'];
        const numberOfObjectsStart = ['zrevrangebyscore', numberOfObjectsKey, start, '-inf', 'LIMIT', '0', '1'];
        const numberOfObjectsEnd = ['zrevrangebyscore', numberOfObjectsKey, end, '-inf', 'LIMIT', '0', '1'];
        const timestampRange = this._getTimestampRange(start, end);
        const metricKeys = [].concat.apply([], timestampRange.map(i => (0, _schema.getKeys)(obj, i)));
        const cmds = metricKeys.map(item => ['get', item]);
        cmds.push(storageUtilizedStart, storageUtilizedEnd, numberOfObjectsStart, numberOfObjectsEnd);
        ...
        datastore.batch(cmds, (err, res) => {
            ...
            const metricResponse = this._getMetricResponse(resource, start, end);
