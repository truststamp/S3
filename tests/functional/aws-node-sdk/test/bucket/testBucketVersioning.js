...
describe.skip('aws-node-sdk test bucket versioning', function testSuite() {
    this.timeout(600000);
    let s3 = undefined;
    const versionIds = [];
    const counter = 100;
        ...
        async.timesSeries(counter, (i, next) => s3.putObject(params,
            (err, data) => {
                ...
                s3.getObject(paramsNull, (err, data) => {
                    ...
                    assert(versionId <= nullVersionId, 'invalid order');
                    next(err);
                });
            }), done);
    });
        ...
        async.waterfall([
            ...
            callback => async.timesSeries(counter, (i, next) =>
                s3.putObject(params, (err, data) => {
                    assert.strictEqual(err, null);
                    versionIds.push(data.VersionId);
                    assert(data.VersionId < nullVersionId, 'new ver. >= null');
                    next();
                }), err => callback(err)),
            callback => s3.getObject(paramsNull, (err, data) => {
                assert.strictEqual(err, null);
